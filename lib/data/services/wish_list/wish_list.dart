...
class WishListApi implements WishListRepository {
  ...
  Future<Either<Failure, SuccessResponseModel>> addToWishList(
      {required TokenModel tokenModel, required AddToWishList addToWishList}) async {
    try {
       _dio.options.headers['Authorization'] = tokenModel.accessToken;
      final response = await _dio.post(ApiEndPoints.addToWishList,data: addToWishList.toJson());
      if (response.statusCode == 200) {
        return Right(SuccessResponseModel.fromJson(response.data));
      } else if (response.statusCode == 500) {
        return Left(Failure.serverFailure(
            message: SuccessResponseModel.fromJson(response.data)
                .message));
      } else {
  ...
  Future<Either<Failure, GetWishlistResponseModel>> getWishlist(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return Left(Failure.serverFailure(
            message: GetWishlistResponseModel.fromJson(response.data)
                .message));
      } else {
        return Left(Failure.clientFailure(
            message: GetWishlistResponseModel.fromJson(response.data)
  ...
  Future<Either<Failure, SuccessResponseModel>> removeFromWishList(
    ...
    try {
       _dio.options.headers['Authorization'] = tokenModel.accessToken;
      final response = await _dio.delete(ApiEndPoints.removeFromWishList,queryParameters: idQurrey.toJson());
      if (response.statusCode == 200) {
        return Right(SuccessResponseModel.fromJson(response.data));
      } else if (response.statusCode == 500) {
        return Left(Failure.serverFailure(
